...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    setState(() {
      _studentName = prefs.getString('student_name') ?? 'Unknown Student';
  ...
  Widget _buildAvatarWidget(ThemeData theme) {
    ...
    if (_avatarUrl != null && _avatarUrl!.isNotEmpty) {
      if (_avatarUrl!.startsWith('/') || _avatarUrl!.startsWith('file://')) {
        ...
        final url = _avatarUrl!.startsWith('http') ? _avatarUrl! : '$baseUrl$_avatarUrl';
        imageWidget = CircleAvatar(
          radius: size / 2,
      ...
      final initials = _studentName.isNotEmpty ? _studentName.substring(0, 1).toUpperCase() : '?';
      imageWidget = CircleAvatar(
        radius: size / 2,
        ...
        Positioned(
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _isUploading ? theme.pSecondaryText : theme.primaryColor,
                shape: BoxShape.circle,
                border: Border.all(color: theme.scaffoldBackgroundColor, width: 3),
                boxShadow: [BoxShadow(color: theme.primaryColor.withOpacity(0.5), blurRadius: 8, spreadRadius: 1)],
              ),
              child: _isUploading
  ...
  Future<void> _pickAndUploadAvatar() async {
    ...
    try {
      // 1. Save Locally Instance (Offline First)
      ...
      _log('Saved avatar locally to: $localPath');

      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('avatar_url', localPath);
      await prefs.setString('pending_avatar_path', localPath);
      
      if (mounted) {
        final userProv = Provider.of<UserProvider>(context, listen: false);
        userProv.updateAvatar(localPath);
        setState(() => _avatarUrl = localPath);
        _showSuccess('Profile updated locally. Syncing in background...');
      }

      // 2. Attempt Background Upload (Silently)
      _log('Attempting silent background upload...');
      try {
        final result = await ApiService.uploadStudentAvatar(localPath);
        _log('Background upload successful: $result');
        ...
        if (serverUrl != null) {
          ...
          if (mounted) {
             ...
             setState(() => _avatarUrl = serverUrl);
          }
        }
      } catch (e) {
        _log('Silent background upload failed (will retry on next sync): $e');
        // No error shown to user as it's meant to be silent.
      }
    } catch (e) {
      _log('Local save failed: $e');
      if (mounted) {
        _showError('Failed to save profile locally: $e');
      }
    } finally {
              ...
              child: ElevatedButton(
                ...
                onPressed: isSaving ? null : () async {
                  ...
                  try {
                    ...
                    await prefs.setString('pending_student_name', newName);
                    ...
                    try {
                      ...
                      _log('Silent name sync failed (will retry on next sync): $e');
                    }
                  } finally {
        ...
        child: Column(
          ...
          children: [
            Center(child: Container(width: 40, height: 4,
              decoration: BoxDecoration(color: theme.pSecondaryText.withOpacity(0.3), borderRadius: BorderRadius.circular(2)))),
            const SizedBox(height: 20),
            Text('CHANGE PASSWORD / PIN', style: TextStyle(color: theme.pPrimaryText, fontWeight: FontWeight.w900, fontSize: 18, letterSpacing: 1.5)),
            const SizedBox(height: 16),
            _pinField(oldPinCtrl, 'Current Password', theme),
            const SizedBox(height: 12),
            _pinField(newPinCtrl, 'New Password', theme),
            const SizedBox(height: 12),
            _pinField(confirmCtrl, 'Confirm New Password', theme),
            const SizedBox(height: 16),
            SizedBox(
              width: double.infinity,
              height: 52,
              child: ElevatedButton(
                ...
                onPressed: isSaving ? null : () async {
                  ...
                  setSheetState(() => isSaving = true);
                  try {
                    await ApiService.updateStudentProfile(password: newPinCtrl.text);
                    // Update stored password for auto-login
                    final prefs = await SharedPreferences.getInstance();
                    await prefs.setString('student_password', newPinCtrl.text);
                    if (mounted) Navigator.pop(ctx);
                    if (mounted) _showSuccess('Password changed successfully!');
                  } catch (e) {
                    if (mounted) _showError('Failed: ${e.toString().replaceFirst("Exception: ", "")}');
                  } finally {
                    if (mounted) setSheetState(() => isSaving = false);
                  }
                },
                child: isSaving ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('CHANGE PASSWORD', style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.5)),
              ),
            ),
            const SizedBox(height: 8),
          ],
  ...
  Widget _pinField(TextEditingController ctrl, String label, ThemeData theme) {
    return TextField(
      controller: ctrl,
      obscureText: true,
      style: TextStyle(color: theme.pPrimaryText, fontWeight: FontWeight.w700),
  ...
  void _handleLogout() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Ensure you have synced any active trip refunds before logging out.',
  ...
  void _handleDeleteAccount() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: Theme.of(context).cardColor,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24), side: BorderSide(color: Colors.redAccent.withOpacity(0.5))),
        title: const Row(children: [Icon(Icons.warning, color: Colors.redAccent), SizedBox(width: 8), Text('DELETE ACCOUNT', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.w900, letterSpacing: 1.2))]),
        content: Text('This is irreversible. All wallet data will be permanently deleted.', style: TextStyle(color: Theme.of(context).pSecondaryText, fontSize: 16, height: 1.5)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: Text('CANCEL', style: TextStyle(color: Theme.of(context).pSecondaryText, fontWeight: FontWeight.bold, fontSize: 16))),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
            onPressed: () async {
              try {
                await ApiService.deleteStudentAccount();
              } catch (_) {}
              final prefs = await SharedPreferences.getInstance();
              await prefs.clear();
              if (!mounted) return;
              Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const StudentEntryScreen()), (route) => false);
            },
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text('STUDENT HUB', style: TextStyle(color: theme.pPrimaryText, fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: 2.5)),
              ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(_studentName,
                            style: TextStyle(color: theme.pPrimaryText, fontSize: 24, fontWeight: FontWeight.w900, height: 1.1),
                          ...
                          Container(
                            ...
                            child: Text('MATRIC: $_studentId',
                              style: TextStyle(color: theme.pAccentText, fontSize: 13, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
                          ),
                        ],
                      ),
                    ),
              ...
              _buildListRow(Icons.shield_outlined, 'Change Password / PIN', theme, onTap: _showChangePinSheet),
              Divider(color: theme.pSecondaryText.withOpacity(0.1), height: 1, indent: 68),
              _buildListRow(Icons.photo_camera_outlined, 'Update Profile Photo', theme, onTap: _pickAndUploadAvatar),
